...
__global__ void aten_mul_scalar_kernel(float* input, float* output) {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 5 loop overheads saved

    // FUSED (6 ops): x=TLOAD(...); result=TMULS(...); output=TSTORE(...); x=TLOAD(...); result=TMULS(...); output=TSTORE(...)
    if (_row < 1 && _col < 4096) {
...
void aten_mul_scalar(float* input, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    aten_mul_scalar_kernel<<<grid, block>>>(input, output);
