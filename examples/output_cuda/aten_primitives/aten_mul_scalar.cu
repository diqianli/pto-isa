...
__global__ void aten_mul_scalar_kernel(float* input, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
...
void aten_mul_scalar(float* input, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    aten_mul_scalar_kernel<<<grid, block>>>(input, output, num_full_tiles, tail_elements, zero, tile_size);
