...
__global__ void prims_div_kernel(float* input_x, float* input_y, float* output) {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 7 loop overheads saved

    // FUSED (8 ops): x=TLOAD(...); y=TLOAD(...); result=TDIV(...); output=TSTORE(...); x=TLOAD(...); y=TLOAD(...); result=TDIV(...); output=TSTORE(...)
    if (_row < 1 && _col < 4096) {
...
void prims_div(float* input_x, float* input_y, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    prims_div_kernel<<<grid, block>>>(input_x, input_y, output);
