...
__global__ void prims_add_kernel(float* input_a, float* input_b, float* output) {
...
void prims_add(float* input_a, float* input_b, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    prims_add_kernel<<<grid, block>>>(input_a, input_b, output);
