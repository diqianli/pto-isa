...
__global__ void prims_add_kernel(float* input_x, float* input_y, float* output) {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 7 loop overheads saved

    // FUSED (8 ops): x=TLOAD(...); y=TLOAD(...); result=TADD(...); output=TSTORE(...); x=TLOAD(...); y=TLOAD(...); result=TADD(...); output=TSTORE(...)
    if (_row < 1 && _col < 4096) {
...
void prims_add(float* input_x, float* input_y, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    prims_add_kernel<<<grid, block>>>(input_x, input_y, output);
