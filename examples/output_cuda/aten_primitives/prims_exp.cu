...
__global__ void prims_exp_kernel(float* input, float* output) {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 5 loop overheads saved

    // FUSED (6 ops): x=TLOAD(...); result=TEXP(...); output=TSTORE(...); x=TLOAD(...); result=TEXP(...); output=TSTORE(...)
    if (_row < 1 && _col < 4096) {
...
void prims_exp(float* input, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    prims_exp_kernel<<<grid, block>>>(input, output);
