...
__global__ void prims_sub_kernel(float* input_x, float* input_y, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
    ...
    for (int tile_idx = 0; tile_idx < num_full_tiles; tile_idx += 1) {
        ...
        // FUSED (4 ops): x=TLOAD(...); y=TLOAD(...); result=TSUB(...); output=TSTORE(...)
        if (_row < 1 && _col < 4096) {
...
void prims_sub(float* input_x, float* input_y, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    prims_sub_kernel<<<grid, block>>>(input_x, input_y, output, num_full_tiles, tail_elements, zero, tile_size);
