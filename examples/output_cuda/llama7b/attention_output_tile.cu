...
__device__ float weights[32][128];
__device__ float v[128][128];
__device__ float result[32][128];

__global__ void attention_output_tile_kernel(float* input_weights, float* input_v, float* output) {
...
void attention_output_tile(float* input_weights, float* input_v, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_output_tile_kernel<<<grid, block>>>(input_weights, input_v, output);
