...
__device__ float cos_pos[32][128];
__device__ float sin_pos[32][128];
__device__ float x_cos[32][128];
__device__ float x_sin[32][128];
__device__ float result[32][128];

__global__ void rope_tile_kernel(float* input, float* cos_cache, float* sin_cache, float* output) {
...
void rope_tile(float* input, float* cos_cache, float* sin_cache, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    rope_tile_kernel<<<grid, block>>>(input, cos_cache, sin_cache, output);
