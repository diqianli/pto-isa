...
__device__ float residual[32][128];
__device__ float result[32][128];

__global__ void residual_add_tile_kernel(float* input, float* input_residual, float* output) {
...
void residual_add_tile(float* input, float* input_residual, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    residual_add_tile_kernel<<<grid, block>>>(input, input_residual, output);
