...
__device__ float c[32][128];

__global__ void tile_matmul_kernel(float* input_a, float* input_b, float* output) {
...
void tile_matmul(float* input_a, float* input_b, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_matmul_kernel<<<grid, block>>>(input_a, input_b, output);
