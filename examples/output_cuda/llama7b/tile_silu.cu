...
__device__ float neg_x[32][128];
__device__ float exp_neg_x[32][128];
__device__ float one_plus_exp[32][128];
__device__ float sigmoid[32][128];
__device__ float result[32][128];

__global__ void tile_silu_kernel(float* input, float* output) {
...
void tile_silu(float* input, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_silu_kernel<<<grid, block>>>(input, output);
