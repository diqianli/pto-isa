...
__device__ float q[32][128];
__device__ float k_t[128][128];
__device__ float scores[32][128];
__device__ float scaled_scores[32][128];

__global__ void attention_score_tile_kernel(float* input_q, float* input_kt, float* output) {
...
void attention_score_tile(float* input_q, float* input_kt, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_score_tile_kernel<<<grid, block>>>(input_q, input_kt, output, scale);
