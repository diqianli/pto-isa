...
void prims_minimum(float* input_x, float* input_y, float* output) {
    ...
    // FUSED LOOP (8 ops): x=TLOAD(input_x,0,0); y=TLOAD(input_y,0,0); result=TMIN(x,y); output=TSTORE(result,0,0); x=TLOAD(input_x,0,0); y=TLOAD(input_y,0,0); result=TMIN(x,y); output=TSTORE(result,0,0)
