...
void aten_div_scalar(float* input, float* output) {
    ...
    // FUSED LOOP (6 ops): x=TLOAD(input,0,0); result=TDIVS(x,4.0f); output=TSTORE(result,0,0); x=TLOAD(input,0,0); result=TDIVS(x,4.0f); output=TSTORE(result,0,0)
    ...
    for (int _row = 0; _row < 1; _row++) {
        ...
        for (_col = 0; _col + 4 <= 4096; _col += 4) {
            ...
            float32x4_t _vr7 = vdivq_f32(_v6, _vs0);
