...
void prims_mul(float* input_x, float* input_y, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
    ...
    for (int tile_idx = 0; tile_idx < num_full_tiles; tile_idx += 1) {
        ...
        // FUSED LOOP (4 ops): x=TLOAD(input_x,tile_idx,0); y=TLOAD(input_y,tile_idx,0); result=TMUL(x,y); output=TSTORE(result,tile_idx,0)
    ...
    if (has_tail) {
        ...
        // FUSED LOOP (4 ops): x=TLOAD(input_x,num_full_tiles,0); y=TLOAD(input_y,num_full_tiles,0); result=TMUL(x,y); output=TSTORE(result,num_full_tiles,0)
        for (int _row = 0; _row < 1; _row++) {
            int _col;
            // Vectorized loop
            for (_col = 0; _col + 4 <= 4096; _col += 4) {
