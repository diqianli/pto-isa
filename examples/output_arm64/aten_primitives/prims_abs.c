...
void prims_abs(float* input, float* output, int32_t num_full_tiles, int32_t tail_elements, int32_t zero, int32_t tile_size) {
    ...
    for (int tile_idx = 0; tile_idx < num_full_tiles; tile_idx += 1) {
        ...
        // FUSED LOOP (3 ops): x=TLOAD(input,tile_idx,0); result=TABS(x); output=TSTORE(result,tile_idx,0)
    ...
    if (has_tail) {
        ...
        // FUSED LOOP (3 ops): x=TLOAD(input,num_full_tiles,0); result=TABS(x); output=TSTORE(result,num_full_tiles,0)
        for (int _row = 0; _row < 1; _row++) {
            int _col;
            // Vectorized loop
            for (_col = 0; _col + 4 <= 4096; _col += 4) {
