...
from compile.pto_dynamic_tiling import compute_tile_shape, get_tile_info, MAX_TILE_BYTES
...
def main():
    """Create and describe the LLaMA 7B module with Flash Attention."""
    print("=" * 70)
    print("PTO LLaMA 7B Layer - Flash Attention")
    print("=" * 70)
    ...
    print(f"  Block Size (Br=Bc): {FLASH_BLOCK_SIZE}")
    print(f"  Total Memory (with reuse): {flash_info['total_with_reuse_kb']:.1f} KB")
    ...
    level1_funcs = [n for n in incore_funcs if n.startswith('tile_')]
    level2_funcs = [n for n in incore_funcs if not n.startswith('tile_') and not n.startswith('flash_')]
    flash_funcs = [n for n in incore_funcs if n.startswith('flash_')]
    
    print(f"    Level 1 (basic ops): {len(level1_funcs)} functions")
    print(f"    Level 2 (composed): {len(level2_funcs)} functions")
    print(f"    Flash Attention: {len(flash_funcs)} functions")
